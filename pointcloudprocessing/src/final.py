...
import pcl_helper
#Noise
def do_statistical_outlier_filtering(cloud , mean_k = 10 , thresh = 0.001):
 outlier_filter = cloud.make_statistical_outlier_filter()
 outlier_filter.set_mean_k(mean_k)
 outlier_filter.set_std_dev_mul_thresh(thresh)
 return outlier_filter.filter()
#Downsampling
def do_voxel_grid_downsampling(pcl_data, leaf_size = 0.01):
...
def callback(input_ros_msg):
 ...
 cloud_roi_x = do_passthrough(cloud_downsampled, 'x', 0.0, 30.0)
 cloud_roi_y = do_passthrough(cloud_roi_x, 'y', -10.0, 10.0)
 _, _, cloud_ransaced = do_ransac_plane_normal_segmentation(cloud_roi_y, 0.01)
